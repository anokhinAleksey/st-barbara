...
class Day(models.Model):
    ...
    @cached_property
    def full_title(self):
        """Full title consists of title and subtitle if it's set."""
        return f"{self.title}: {self.subtitle}" if self.subtitle else self.title


class Reading(models.Model):
    """Model for reading entity.
...
class Pericope(models.Model):
    ...
    async def aget_passage(self, language="en"):
        try:
            ...
            self.passage = [
                verse async for verse in self.get_passage(language=language)
            ]
            return self.passage
    ...
    def get_passage(self, language="en"):
        ...
        if match:
            return Composite.objects.filter(composite_num=match.group(1)).annotate(
